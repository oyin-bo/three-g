 ...
 */
test('quadrupole-kernels.large-scale: 100 particles remain stable over 100 steps', async () => {
 ...
 */
test('quadrupole-kernels.large-scale: 1000 particles clustered remain bound', async () => {
  const { canvas, gl } = createTestCanvas();
  ...
  // Reasonable upper bound on speeds for gravitationally bound system
  assert.ok(maxSpeed < 5.0, `Max speed should be reasonable: ${maxSpeed.toFixed(3)} < 5.0`);
...
/**
 * Test 3: Quadrupole accuracy advantage shows in large systems
 */
test('quadrupole-kernels.large-scale: quadrupole provides better accuracy for 500 particles', async () => {
  const { canvas, gl } = createTestCanvas();
  ...
  // Step multiple times and verify stability metrics improve
  let prevTotalKE = 0;
  ...
  for (let step = 0; step < 150; step++) {
    ...
    if (step % 10 === 0) {
      ...
      let KE = 0;
      for (let i = 0; i < particleCount; i++) {
        const vx = data.velocities[i * 4 + 0];
        const vy = data.velocities[i * 4 + 1];
        const vz = data.velocities[i * 4 + 2];
        const mass = data.positions[i * 4 + 3];
        KE += 0.5 * mass * (vx*vx + vy*vy + vz*vz);
      }
      
      if (step > 0) {
        energyVariation += Math.abs(KE - prevTotalKE);
      }
      prevTotalKE = KE;
    }
  }
  
  // Quadrupole should show stable energy evolution
  const avgEnergyVariation = energyVariation / 14; // 14 samples after first
  assert.ok(avgEnergyVariation < 100.0, 
    `Energy variation should be low (quadrupole advantage): avgVar=${avgEnergyVariation.toFixed(2)}`);
