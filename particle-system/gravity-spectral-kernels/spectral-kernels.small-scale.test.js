 ...
 */
test('spectral-kernels.small-scale: single particle at rest remains stationary', async () => {
 ...
 */
test('spectral-kernels.small-scale: two particles attract each other', async () => {
  const { canvas, gl } = createTestCanvas();
 ...
 */
test('spectral-kernels.small-scale: ten particles in cluster contract inward', async () => {
  const { canvas, gl } = createTestCanvas();
...
/**
 * Test 4: Grid resolution convergence (spectral-specific)
 */
test('spectral-kernels.small-scale: higher grid resolution improves accuracy', async () => {
  const { canvas, gl } = createTestCanvas();
