 ...
 */
test('spectral-kernels.conservation: energy approximately conserved over time', async () => {
  ...
  const system = new ParticleSystemSpectralKernels({
    ...
    gridSize: 32 // PM grid resolution
  });
 ...
 */
test('spectral-kernels.conservation: momentum conserved in isolated system', async () => {
  const { canvas, gl } = createTestCanvas();
 ...
 */
test('spectral-kernels.conservation: angular momentum conserved in rotating system', async () => {
  const { canvas, gl } = createTestCanvas();
