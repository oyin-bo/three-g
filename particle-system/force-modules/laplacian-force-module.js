...
export class LaplacianForceModule {
  ...
  createTextures() {
    ...
    // colIdx texture (E elements)
    const colIdxSize = this.calculateTextureSize(this.colIdx.length);
    this.colIdxTex = this.createDataTexture(colIdxSize.width, colIdxSize.height, gl.R32F, gl.RED, gl.FLOAT);
    
    // weight texture (E elements)
    const weightSize = this.calculateTextureSize(this.weight.length);
    this.weightTex = this.createDataTexture(weightSize.width, weightSize.height, gl.R32F, gl.RED, gl.FLOAT);
    
    // shards texture (numShards * 4 for RGBA packing: nodeId, start, len, 0)
    const shardsSize = this.calculateTextureSize(this.numShards);
    this.shardsTex = this.createDataTexture(shardsSize.width, shardsSize.height, gl.RGBA32F, gl.RGBA, gl.FLOAT);
    
    // deg texture
    const degSize = this.calculateTextureSize(this.particleCount);
    this.degTex = this.createDataTexture(degSize.width, degSize.height, gl.R32F, gl.RED, gl.FLOAT);
    
    if (this.degInv) {
      this.degInvTex = this.createDataTexture(degSize.width, degSize.height, gl.R32F, gl.RED, gl.FLOAT);
    }
    
    // partials textures (numShards, ping-pong for reduction)
    const partialsSize = this.calculateTextureSize(this.numShards);
    this.partialsTex = this.createRenderTexture(partialsSize.width, partialsSize.height);
    this.partialsTexPingPong = this.createRenderTexture(partialsSize.width, partialsSize.height);
  ...
  uploadData() {
    ...
    gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, colIdxSize.width, colIdxSize.height, gl.RED, gl.FLOAT, colIdxPadded);
    ...
    gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, weightSize.width, weightSize.height, gl.RED, gl.FLOAT, weightPadded);
    
    // Upload shards (pack as RGBA: nodeId, start, len, 0)
    const shardsSize = this.calculateTextureSize(this.numShards);
    const shardsPacked = new Float32Array(shardsSize.width * shardsSize.height * 4);
    ...
    gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, shardsSize.width, shardsSize.height, gl.RGBA, gl.FLOAT, shardsPacked);
    ...
    gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, degSize.width, degSize.height, gl.RED, gl.FLOAT, degPadded);
    ...
    if (this.degInv && this.degInvTex) {
      ...
      gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, degSize.width, degSize.height, gl.RED, gl.FLOAT, degInvPadded);
    }
  ...
  createPrograms() {
    ...
    this.partialsProgram = this.createProgram(this.getFullscreenVS(), this.getPartialsFS());
    
    // Pass 2: reduction (blending variant)
    this.reduceBlendProgram = this.createProgram(this.getPointVS(), this.getReduceBlendFS());
    
    // Pass 3: laplacian finish
    this.laplacianFinishProgram = this.createProgram(this.getFullscreenVS(), this.getLaplacianFinishFS());
  }
  ...
  accumulate(ctx) {
    ...
    if (this.disableFloatBlend) {
      ...
      console.warn('[LaplacianForce] Segmented reduction not yet implemented, skipping');
      return;
    } else {
  ...
  runPartialsPass(positionTex) {
    ...
    // Uniforms
    const shardsSize = this.calculateTextureSize(this.numShards);
    gl.uniform2i(gl.getUniformLocation(prog, 'uShardSize'), shardsSize.width, shardsSize.height);
    
    const colIdxSize = this.calculateTextureSize(this.colIdx.length);
    gl.uniform2i(gl.getUniformLocation(prog, 'uColIdxSize'), colIdxSize.width, colIdxSize.height);
    
    const posSize = { width: this.textureWidth, height: this.textureHeight };
    gl.uniform2i(gl.getUniformLocation(prog, 'uPosSize'), posSize.width, posSize.height);
    
    gl.uniform1i(gl.getUniformLocation(prog, 'uShardBlockSize'), this.options.shardSize);
  ...
  runReduceBlendPass() {
    ...
    const shardsSize = this.calculateTextureSize(this.numShards);
    gl.uniform2i(gl.getUniformLocation(prog, 'uShardSize'), shardsSize.width, shardsSize.height);
    gl.uniform2i(gl.getUniformLocation(prog, 'uAxSize'), AxSize.width, AxSize.height);
    gl.uniform2i(gl.getUniformLocation(prog, 'uPartialsSize'), shardsSize.width, shardsSize.height);
  ...
  runLaplacianFinishPass(targetForce, positionTex) {
    ...
    const degSize = this.calculateTextureSize(this.particleCount);
    gl.uniform2i(gl.getUniformLocation(prog, 'uDegSize'), degSize.width, degSize.height);
    
    gl.uniform2i(gl.getUniformLocation(prog, 'uPosSize'), this.textureWidth, this.textureHeight);
  ...
  dispose() {
    ...
    if (this.partialsFBOPingPong) gl.deleteFramebuffer(this.partialsFBOPingPong);
    ...
    if (this.reduceSegmentedProgram) gl.deleteProgram(this.reduceSegmentedProgram);
    if (this.laplacianFinishProgram) gl.deleteProgram(this.laplacianFinishProgram);
