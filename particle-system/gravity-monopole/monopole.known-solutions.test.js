 ...
 * @returns {Array<{position: [number,number,number], velocity: [number,number,number]}>}
 */
function sampleTrajectory(system, particleIndex, numSteps, sampleInterval) {
  const samples = [];
  
  for (let step = 0; step < numSteps; step++) {
    if (step % sampleInterval === 0) {
      const data = readParticleData(system, particleIndex);
      samples.push({
 ...
 */
test('monopole.known-solutions: binary circular orbit maintains separation', async () => {
 ...
 */
test('monopole.known-solutions: binary eccentric orbit shows periapse/apoapse', async () => {
  const { canvas, gl } = createTestCanvas();
 ...
 */
test('monopole.known-solutions: free fall matches analytical solution', async () => {
  const { canvas, gl } = createTestCanvas();
