 ...
 */
test('quadrupole-kernels.small-scale: single particle at rest remains stationary', async () => {
 ...
 */
test('quadrupole-kernels.small-scale: two particles attract each other', async () => {
  const { canvas, gl } = createTestCanvas();
 ...
 */
test('quadrupole-kernels.small-scale: three-body equilateral triangle maintains shape', async () => {
  const { canvas, gl } = createTestCanvas();
  ...
  // Quadrupole should maintain better stability
  const tolerance = 0.25; // 25% tolerance (better than monopole's 30%)
  for (let i = 0; i < 3; i++) {
 ...
 */
test('quadrupole-kernels.small-scale: ten particles in cluster contract inward', async () => {
  const { canvas, gl } = createTestCanvas();
 ...
 */
test('quadrupole-kernels.small-scale: empty system does not crash', async () => {
  const { canvas, gl } = createTestCanvas();
