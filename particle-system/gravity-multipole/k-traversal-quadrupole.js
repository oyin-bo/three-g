...
out vec4 fragColor;`;
  ...
  const buildSampler = (name) => {
    const cases = Array.from({ length: maxL }, (_, i) => `if (level == ${i}) return texelFetch(${name}_${i}, coord, 0);`).join('\n  else ');
    return `
...
void main() {
  ...
  return [samplerDecl, buildSampler('u_levelA0'), buildSampler('u_levelA1'), buildSampler('u_levelA2'), body].join('\n');
}
