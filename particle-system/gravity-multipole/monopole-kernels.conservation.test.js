 ...
 */
test('monopole-kernels.conservation: energy approximately conserved over time', async () => {
  ...
  // Allow some drift due to numerical integration
  assert.ok(relDrift < 0.2, 
    `Energy should be approximately conserved: initial=${initialE.toExponential(3)}, final=${finalE.toExponential(3)}, relDrift=${relDrift.toFixed(3)}`);
 ...
 */
test('monopole-kernels.conservation: momentum conserved in isolated system', async () => {
  const { canvas, gl } = createTestCanvas();
 ...
 */
test('monopole-kernels.conservation: angular momentum conserved in rotating system', async () => {
  const { canvas, gl } = createTestCanvas();
  ...
  assert.ok(relChange < 0.15, 
    `Angular momentum should be approximately conserved: initial=[${initialL.map(x=>x.toFixed(3))}], final=[${finalL.map(x=>x.toFixed(3))}], relChange=${relChange.toFixed(3)}`);
