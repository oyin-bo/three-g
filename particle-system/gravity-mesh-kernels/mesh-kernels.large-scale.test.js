...
/**
 * Test 1: 100 particles
 */
test('mesh-kernels.large-scale: 100 particles evolve without errors', async () => {
...
/**
 * Test 2: 256 particles
 */
test('mesh-kernels.large-scale: 256 particles perform efficiently', async () => {
  const { canvas, gl } = createTestCanvas();
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    const v = Math.sqrt(0.0003 * 10 / r) * 0.5;
  ...
  assert.ok(msPerStep < 50, 
    `Mesh method should be efficient: ${msPerStep.toFixed(2)}ms per step`);
...
/**
 * Test 3: 1000 particles
 */
test('mesh-kernels.large-scale: 1000 particles remain stable', async () => {
  const { canvas, gl } = createTestCanvas();
  ...
  const system = new ParticleSystemMeshKernels({
    ...
    softening: 0.1,
    mesh: { gridSize: 128, assignment: 'cic' }
  });
...
/**
 * Test 4: Clustering behavior
 */
test('mesh-kernels.large-scale: particles cluster over time', async () => {
  const { canvas, gl } = createTestCanvas();
...
/**
 * Test 5: Grid resolution scaling
 */
test('mesh-kernels.large-scale: higher grid resolution handles more particles', async () => {
  const { canvas, gl } = createTestCanvas();
