...
```
// <executing-page> | <HH:MM:SS> | executing | "<first 50 chars of code, newlines→space, … if long>"
// <other-page> | <HH:MM:SS> | <state>
// <other-page> | <HH:MM:SS> | <state>
// proceeding... job=<id> started=<ISO>
```
...
- The body becomes a single acknowledgement line.

### 4) Success (result written back)

```
// <page> | <HH:MM:SS> | completed ok (<duration>) | result below
// <other-page> | <HH:MM:SS> | <state>
// <other-page> | <HH:MM:SS> | <state>
var result = <JSON-serialized result or String(value)>;
// finished=<ISO> duration=<ms>ms
```

Notes:

- The just-completed page remains pinned as the first header line (state: completed ok) until a new request is issued.
- The body contains a single `var result = ...` assignment plus a trailing comment with finish time and duration.

### 5) Error (stack verbatim)

```
// <page> | <HH:MM:SS> | completed error (<duration>) | error below
// <other-page> | <HH:MM:SS> | <state>
// <other-page> | <HH:MM:SS> | <state>
...
- The completed header line stays as-is (not wiped back to idle) until the actor writes a new request.
- All pages continue polling; their `last` timestamps in the header keep updating on a periodic refresh.
- The body (result or stack) stays until overwritten by the next request.

## Header Format and Ordering

- Pattern:
  - Executing: `// <name> | <HH:MM:SS> | executing | "<snippet>"`
  - Completed OK: `// <name> | <HH:MM:SS> | completed ok (<duration>) | result below`
  - Completed Error: `// <name> | <HH:MM:SS> | completed error (<duration>) | error below`
  - Idle: `// <name> | <HH:MM:SS> | idle`
- **Pinned first line**:
  - If executing: the executing page is first.
  - Else if last-completed exists: that page is first (completed ok/error).
...
- `duration = finishedAt - startedAt` displayed:
  ...
  - In body as `// finished=<ISO> duration=<ms>ms`.
...
```
// index-7-zen-12:01:03 | 09:33:25 | idle
// simplistic-5-nova-09:12:45 | 09:33:23 | idle
// texture-11-echo-13:22:55 | 09:32:58 | idle
```
...
```javascript
// index
(async () => { window.setMethod && window.setMethod('spectral'); return 'ok'; })()
```

### Acknowledge (serve.js rewrites)

```
// index-7-zen-12:01:03 | 09:34:01 | executing | "(async () => { window setMethod && window..."
// proceeding... job=12 started=2025-10-12T08:34:01.231Z
```

### Success (serve.js rewrites)

```
// index-7-zen-12:01:03 | 09:34:02 | completed ok (742ms) | result below
var result = "ok";
// finished=2025-10-12T08:34:02.0Z duration=742ms
```

### Error (serve.js rewrites)

```
// index-7-zen-12:01:03 | 09:34:10 | completed error (1.22s) | error below
TypeError: Cannot read properties of undefined (reading 'fft')
    at <stack...>
    ...
// finished=2025-10-12T08:34:10.7Z duration=1220ms
```
...
The file-based REPL is an additive pathway to the existing HTTP REPL (`docs/7-http-test-harness.md`). Both share the same page registry and execution engine, differing only in how jobs are queued and where results are surfaced.
