````markdown
# Markdown-based REPL Implementation: Daebug

This document describes the markdown-based REPL format that has been implemented as **daebug**. This replaces the legacy HTTP REPL described in older versions of [7-http-test-harness.md](7-http-test-harness.md) and [7.1-file-test-harness.md](7.1-file-test-harness.md).

Daebug turns REPL from transactional request-response into a chat-like log format. Files extend downward, preserving all past interactions.

This format is LLM-friendly: file edit tools work smoothly with append-only content, and history tracking is built-in.

The format uses Markdown. Example session file:
...
## Implementation: Daebug

Daebug implements the markdown-based REPL format described above. It preserves human-first design: no job ids, no machine metadata, readable timestamps, multiple concurrent sessions, and clear formatting.
