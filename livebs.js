...
import * as THREE from "three";
import { createScene } from "three-pop";
import { massSpotMesh, particleSystem } from "./index.js";
...
/** @type {import('./gravity/multipole/gravity-monopole.js').GravityMonopole} */
const physics = particleSystem({
  gl: renderer.getContext(),
  gravityStrength: 0.0003,
  dt: 1 / 60,
  softening: 0.2,
  particles: Array.from({ length: MAX_PARTICLES }, (_, i) => ({
    x: i < 20 ? i * 0.001 : NaN,
    y: i < 20 ? i * 0.001 : NaN,
    z: i < 20 ? i * 0.001 : NaN,
    mass: i < 20 ? 0.00000001 : 0
  })),
  method: 'monopole',
...
const { mesh } = massSpotMesh({
  ...
  textures: {
    ...
    size: [physics.textureWidth, physics.textureHeight]
  },
...
function animate() {
  physics.step();
  mesh.material.uniforms.u_positionTexture.value = physics.positionTexture;
...
async function pumpFirehose() {
  for await (const chunk of firehose()) {
    for (const event of chunk) {
      ...
      const seenTypes = [];
      if (!seenTypes.includes(event.$type)) {
